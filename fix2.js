...
function getActionButtons(c) {
    if (c.status === 'Liquidado') return \\\
        <button class="icon-btn small" onclick="viewCronograma('\')" title="Ver Cronograma"><i class="ph ph-calendar-dots"></i></button>
    \\\;
    return \\\
        <button class="icon-btn small" onclick="notifyWhatsApp('\')" title="Notificar"><i class="ph ph-whatsapp-logo"></i></button>
        <button class="icon-btn small" onclick="openPaymentModal('\')" title="Registrar Pago de Cuota"><i class="ph ph-currency-dollar"></i></button>
        <button class="icon-btn small" onclick="viewCronograma('\')" title="Ver Cronograma"><i class="ph ph-calendar-dots"></i></button>
        <button class="icon-btn small" onclick="markAsPaid('\')" title="Liquidar Total"><i class="ph ph-check-circle"></i></button>
        <button class="icon-btn small" onclick="editClient('\')" title="Editar"><i class="ph ph-pencil"></i></button>
        <button class="icon-btn small" onclick="deleteClient('\')" title="Eliminar"><i class="ph ph-trash"></i></button>
    \\\;
}
...
window.viewCronograma = function(clientId) {
    ...
    tbody.innerHTML = c.installments.map(inst => {
        ...
        return \\\
            <tr style="border-bottom: 1px solid rgba(255,255,255,0.05); color: \; opacity: \;">
                <td style="padding: 10px;">\</td>
                <td style="padding: 10px;">\</td>
                <td style="padding: 10px;">\</td>
                <td style="padding: 10px; font-weight: \;">\</td>
            </tr>
        \\\;
    }).join('');
        ...
        tbodyPagos.innerHTML = payments.map(p => \\\
            <tr style="border-bottom: 1px solid rgba(255,255,255,0.05);">
                ...
                <td style="padding: 10px; color: var(--success); display:flex; justify-content:space-between; align-items:center;">
                    \
                    <button class="icon-btn small" onclick="sendWhatsAppTicket('\', '\')" title="Enviar Ticket por WhatsApp"><i class="ph ph-whatsapp-logo"></i></button>
                </td>
            </tr>
        \\\).join('');
    }
        ...
        <tr>
            <td>
                <div class="client-cell">
                    ...
                    <div><strong>\</strong><span class="id-text">\</span></div>
                </div>
            </td>
            <td>\</td>
            <td>\</td>
            <td>\</td>
            <td><strong>\</strong><br><span style="font-size: 11px; opacity: 0.7;">\</span></td>
            <td>\</td>
            <td style="display: flex; gap: 4px; justify-content: flex-end;">\</td>
        </tr>
    \\\}).join('');
}
    ...
    tbody.innerHTML = history.map(c => \\\
        <tr>
            ...
            <td style="display: flex; gap: 4px; justify-content: flex-end;">
                <button class="icon-btn small" onclick="viewCronograma('\')" title="Ver Cronograma"><i class="ph ph-calendar-dots"></i></button>
                <button class="icon-btn small" onclick="deleteClient('\')" title="Eliminar del Historial"><i class="ph ph-trash"></i></button>
            </td>
        </tr>
    \\\).join('');
}
        ...
        if (c.status === 'En Mora') {
            alertsHtml += \\\
                <div class="alert-item" style="border-left: 3px solid var(--danger);">
                    ...
                    <button class="icon-btn small" onclick="notifyWhatsApp('\')"><i class="ph ph-whatsapp-logo"></i></button>
                </div>\\\;
        } else if (diffDays >= 0 && diffDays <= 3) {
            let limitText = diffDays === 0 ? 'Vence HOY' : \\\Vence en \ días\\\;
            alertsHtml += \\\
                <div class="alert-item" style="border-left: 3px solid var(--warning);">
                    ...
                    <div class="alert-info">
                        <strong style="color: var(--warning);">\</strong>
                        <span>\ - Cuota: \</span>
                    </div>
                    <button class="icon-btn small" onclick="notifyWhatsApp('\')"><i class="ph ph-whatsapp-logo"></i></button>
                </div>\\\;
